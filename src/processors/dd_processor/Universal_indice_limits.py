...
class Universal_indices_limits():
    ...
    def indices_limit(self):
        for i in self.key:
            self.independent_variables.append(i)
        ...
        for i in self.key:
            self.independent_variables.remove(i)
        ...
        if len(dataframe.columns)>2:
            ...
            pls_reg_2.fit(dataframe[x],dataframe[i])
            pls_col=[]
            y_col=[]
            for j in range(1,len(x)+1):
                pls_col.append("plsscore"+str(j))
            
            pls_dataframe = pd.DataFrame(data = pls_reg_2.x_scores_, columns = pls_col)
            
            
            
            pls_dataframe[i]=dataframe[i]
