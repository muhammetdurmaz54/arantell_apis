...
class MainDB():
    ...
    def create_base_dataframe(self,main_dict_list):
        ...
        dft_low,dft_high=self.vsl_load_conversion(self.main_data_list)
        if len(dft_low)>1 and len(dft_high)>1:
            ...
            print("finish:",main_dict_list['processed_daily_data']['vsl_load_bal']['processed'])
    ...
    def final_maindb_config(self,main_dict_list):
        maindb = database.get_collection("Main_db")
        print("REPORTED DATE!!!!!!!!!!!!!!", self.report_date)
        try:
            maindb.delete_one({"ship_imo": self.ship_imo, "processed_daily_data.rep_dt.processed":self.report_date, "timestamp": self.timestamp})
        except:
            pass
