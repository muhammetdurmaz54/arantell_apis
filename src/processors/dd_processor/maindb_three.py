...
from src.processors.dd_processor.sister_vessel_prediction import Sister_Vessel_pred
...
# indices_preds_main_dict = maindb.find({"ship_imo": 9205926})
...
class MainDB():
    ...
    def sister_vessel_pred(self,main_dict_list):
        sister_vessel_list=self.ship_configs['sister_vessel_list']
        similar_vessel_list=self.ship_configs['similar_vessel_list']
        ...
        for i in sister_vessel_list:
            try:
                ...
                ident_list=[*self.ship_configs['data']]
                temp_list=[]
                for j in ident_list:
                    main_data =maindb.find({"ship_imo":i},{"processed_daily_data."+j+".processed":1,"_id":0})
                ...
                for j in range(len(temp_list)):
                    temp_list_2=[]
                    for k in range(0,len(temp_list[j])):
                        try:
                            temp_list_2.append(temp_list[j][k]['processed_daily_data'][ident_list[j]]['processed'])
                        except:
                            try:
                                temp_list_2.append(temp_list[j][k]['independent_indices'][ident_list[j]]['processed'])
                            except:
        ...
        for i in range(0,len(main_dict_list)):
            ...
            for key in ml_control:
                if key in main_data_dict and pandas.isnull(main_data_dict[key]['processed'])==False:
                    ...
                    val2=[]
                    for col in val:
                        if col in dataframe.columns and col in main_data_dict.keys() and pandas.isnull(main_data_dict[col]['processed']) == False:
                            val2.append(col)
             
                    temp_data=dataframe[val2]
                    temp_data[key]=dataframe[key]
                    curr_date=main_data_dict['rep_dt']['processed'].date()
                    
                    pred_obj=Sister_Vessel_pred()
    ...
    def vsl_load_conversion(self,main_dict_list):
        ...
        if len(dft_low)>1 and len(dft_high)>1:
            ...
            for i in range(0,len(main_dict_list)):
                ...
                if pandas.isnull(main_dict_list[i]['processed_daily_data']['draft_mean']['processed'])==False and pandas.isnull(main_dict_list[i]['processed_daily_data']['vsl_load_bal']['processed'])==False:
                    if main_dict_list[i]['processed_daily_data']['draft_mean']['processed'] < dft_low_mean + 0.35*(dft_high_mean-dft_low_mean):
                        main_dict_list[i]['processed_daily_data']['vsl_load_bal']['processed']=0
                        main_dict_list[i]["vessel_loaded_check"]="Ballast"
                    elif main_dict_list[i]['processed_daily_data']['draft_mean']['processed'] > dft_low_mean + 0.65*(dft_high_mean-dft_low_mean):
    ...
    def create_base_dataframe(self,main_dict_first_list,flag):
        ...
        for j in range(0,len(ident_list)):
            temp_list_2=[]
            for i in range(len(main_dict_list)):
                if main_dict_list[i]['within_good_voyage_limit']==True:
                    try:
                        temp_list_2.append(main_dict_list[i]['processed_daily_data'][ident_list[j]]['processed'])
                    except:
                        try:
                            temp_list_2.append(main_dict_list[i]['independent_indices'][ident_list[j]]['processed'])
                        except:
                            temp_list_2.append(None)
    ...
    def update_indices(self,main_dict_list):
        ...
        for i in range(0,len(main_dict_list)):
            ...
            main_data_dict=main_dict_list[i]['processed_daily_data']
            indice=Indice_Processing(self.ship_configs,main_dict_list[i],self.ship_imo)
            indices_main={}
            ...
            main_dict_list[i]['independent_indices']=indices_main
    ...
    def ewma_limits(self,main_dict_list):
        ...
        alpha=[0.2,0.1,0.05]
        alpha_str={"0.2":"zero_two","0.1":"zero_one","0.05":"zero_zero_five"}
        ewma_final_limits={}
        spe_final_limits={}
        for j in ml_control:
            ewma_limits={}
                    ...
                    try:
                        ...
                        for k in range(0,len(main_dict_list)):
                            if main_dict_list[k]['within_good_voyage_limit']==True:
                                try:
                                    temp_list_2.append(main_dict_list[k]['processed_daily_data'][j]['SPEy'][month])
                                except KeyError:
                                    temp_list_2.append(None)
                        ...
                        spe_data=spe_data.dropna()
                        spe_data=spe_data.reset_index(drop=True)
                        spe_std_val=np.std(spe_data['spe'])
                        ...
                        for alphas in alpha:
                            ...
                            spe_y_limit_array[alpha_str[str(alphas)]]=spe_limit_g_val
                    

                        ewma_limit_list=[]
                        for e_ran in range(0,3):
                            ewma_limit_innerlist=[]
                            temp_list_3=[]
                            for k in range(0,len(main_dict_list)):
                                if main_dict_list[k]['within_good_voyage_limit']==True:
                                    try:
                                        temp_list_3.append(main_dict_list[k]['processed_daily_data'][j]['ewma'][month][e_ran])
                                    except KeyError:
                                        temp_list_3.append(None)
                                    except TypeError:
                                        temp_list_3.append(None)
                            ewma_data=pandas.DataFrame({"ewma":temp_list_3})
                            ewma_data=ewma_data.dropna()
                            ewma_data=ewma_data.reset_index(drop=True)
                            ewma_data = ewma_data[ewma_data['ewma'] <= 8]
                            ewma_data=ewma_data.dropna()
                            ewma_data=ewma_data.reset_index(drop=True)
                            ewma_std_val=np.std(ewma_data['ewma'])
                            ewma_mean_val=np.mean(ewma_data['ewma'])
                            ewma_var_sped=np.var(ewma_data['ewma'])         
                            for alphas in alpha:
                                ewma_obj=EWMA()
                                ewma_obj.fit(ewma_data['ewma'],0.2,ewma_mean_val)
                                L=st.norm.ppf(1-alphas)
                                ewma_val_cal,ewma_ucl_cal,ewma_lcl_cal=ewma_obj.ControlChart(L=L,sigma=ewma_std_val)
                                ewma_limit_innerlist.append(ewma_ucl_cal[-1])
                            ewma_limit_list.append(ewma_limit_innerlist)
                        
                        ewma_limits[month]=ewma_limit_list
                        spe_limits[month]=spe_y_limit_array
                        # print(j,month,"ewmaaaaaaa",ewma_limits)
                        # print(j,month,"speeeeeeee",spe_limits)
        ...
        self.ship_configs['spe_limits']=spe_final_limits
        self.ship_configs['ewma_limits']=ewma_final_limits
        print(self.ship_configs['ewma_limits'])
        print(self.ship_configs['spe_limits'])
        print("kiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
    ...
    def indice_ewma_limit(self,main_dict_list):
                    ...
                    for month in months:
                        try:
                            ...
                            for k in range(0,len(main_dict_list)):
                                if main_dict_list[k]['within_good_voyage_limit']==True:
                                    try:
                                        temp_list_2.append(main_dict_list[k]['independent_indices'][j]['SPEy'][month])
                                    except KeyError:
                                        temp_list_2.append(None)
                            spe_data=pandas.DataFrame({"spe":temp_list_2})
                            spe_data=spe_data.dropna()
                            ...
                            spe_data=spe_data.dropna()
                            spe_data=spe_data.reset_index(drop=True)
                            spe_mean_val=np.mean(spe_data['spe'])
                            spe_var_sped=np.var(spe_data['spe'])
                            spe_h_val=2*(spe_mean_val**2)/(spe_var_sped)
                            spe_g_val=(spe_var_sped)/(2*(spe_mean_val))
                            spe_y_limit_array={}
                            for alphas in alpha:
                                chi_val=st.chi2.ppf(1-alphas, spe_h_val)
                                spe_limit_g_val=spe_g_val*chi_val
                            ...
                            for k in range(0,len(main_dict_list)):
                                if main_dict_list[k]['within_good_voyage_limit']==True:
                                    try:
                                        temp_list_3.append(main_dict_list[k]['independent_indices'][j]['mewma_val'][month])
                                    except KeyError:
                                        temp_list_3.append(None)
                            mewma_data=pandas.DataFrame({"mewma":temp_list_3})
                            mewma_data=mewma_data.dropna()
                            mewma_data=mewma_data.reset_index(drop=True)
                            mewma_data = mewma_data[mewma_data['mewma'] <= 8]
                            mewma_data=mewma_data.dropna()
                            mewma_data=mewma_data.reset_index(drop=True)
                            mewma_std_val=np.std(mewma_data['mewma'])
                            mewma_mean_val=np.mean(mewma_data['mewma'])
                            mewma_limit_list=[]
                            for alphas in alpha:
                                mewma_obj=EWMA()
                                mewma_obj.fit(mewma_data['mewma'],0.2,mewma_mean_val)
                                L=st.norm.ppf(1-alphas)
                                mewma_val_cal,mewma_ucl_cal,mewma_lcl_cal=mewma_obj.ControlChart(L=L,sigma=mewma_std_val)
                                mewma_limit_list.append(mewma_ucl_cal[-1])

                            mewma_limits[month]=mewma_limit_list 
                            spe_limits[month]=spe_y_limit_array
                            # print(j,month,"ewmaaaaaaa",mewma_limits)
                            # print(j,month,"speeeeeeee",spe_limits)
                        except Exception as e:
                            print(e)
        ...
        self.ship_configs['spe_limits_indices']=spe_indices_limits
        self.ship_configs['mewma_limits']=mewma_final_limits
        print(self.ship_configs['mewma_limits'])
        print(self.ship_configs['spe_limits_indices'])
        print("kiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii")
    ...
    def universal_limit(self):
        ...
        try:
            ...
            pass
        self.ship_configs['t2_limits']=t2_limit_dict
    ...
    def anamolies_by_config(self,main_dict_list):
        ...
        for i in range(0,len(main_dict_list)):
            ...
            processed_daily_data=main_dict_list[i]['processed_daily_data']
            independent_indices=main_dict_list[i]['independent_indices']
            try:
                for key in ml_list:
                    try:
                        if key in main_dict_list[i]['processed_daily_data']:
                            spe_anamoly={}
                            t2_anamoly={}
                            ewma_anamoly={}
                            spe_messages={}
                            ...
                            for month in months:
                                ...
                                t2_list=[]
                                ewma_list=[]
                                if pandas.isnull(main_dict_list[i]['processed_daily_data'][key]['SPEy'][month])==False:
                                    for alphas in alpha:
                                        if spe_limits[key][month][alphas]>main_dict_list[i]['processed_daily_data'][key]['SPEy'][month]:
                                ...
                                if pandas.isnull(main_dict_list[i]['processed_daily_data'][key]['t2_initial'][month])==False:
                                    for alphas in alpha:
                                        if t2_limits[key][alphas]>main_dict_list[i]['processed_daily_data'][key]['t2_initial'][month]:
                                            t2_list.append(True)
                                        else:
                                ...
                                try:
                                    if len(main_dict_list[i]['processed_daily_data'][key]['ewma'][month])>0:
                                        for j in range(0,3):
                                            if ewma_limits[key][month][j]>main_dict_list[i]['processed_daily_data'][key]['ewma'][month][j]:
                ...
                for key in indice_list:
                    try:
                        if key in main_dict_list[i]['independent_indices']:
                            t2_indice_anamoly={}
                            spe_indice_anamoly={}
                            mewma_anamoly={}
                            t2_indice_messages={}
                            ...
                            for month in months:
                                ...
                                t2_indice_list=[]
                                mewma_list=[]
                                if pandas.isnull(main_dict_list[i]['independent_indices'][key]['SPEy'][month])==False:
                                    for alphas in alpha:
                                        if spe_indice_limits[key][month][alphas]>main_dict_list[i]['independent_indices'][key]['SPEy'][month]:
                                ...
                                if pandas.isnull(main_dict_list[i]['independent_indices'][key]['t2_initial'][month])==False:
                                    for alphas in alpha:
                                        if t2_indice_limits[key][alphas]>main_dict_list[i]['independent_indices'][key]['t2_initial'][month]:
                                ...
                                if pandas.isnull(main_dict_list[i]['independent_indices'][key]['mewma_val'][month])==False:
                                    for j in range(0,3):
                                        if mewma_limits[key][month][j]>main_dict_list[i]['independent_indices'][key]['mewma_val'][month]:
                ...
                continue
            main_dict_list[i]['processed_daily_data']=processed_daily_data
            main_dict_list[i]['independent_indices']=independent_indices
    ...
    def universal_indices_limits(self):
        ...
        self.ship_configs['t2_limits_indices']=t2_indice_limit
        print(self.ship_configs['t2_limits_indices'])

    def update_cp_msg(self,main_dict_list):
        main_fuel_cp_olmax=main_dict_list[len(main_dict_list)-1]['processed_daily_data']['main_fuel_cp']['operational_limit_value']['max']
        w_force_olmin=main_dict_list[len(main_dict_list)-1]['processed_daily_data']['w_force_cp']['operational_limit_value']['min']
        for i in range(0,len(main_dict_list)):
            print("boooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooom",i)
            main_data=main_dict_list[i]
            try:
                if pandas.isnull(main_data['processed_daily_data']['main_fuel']['processed'])==False and pandas.isnull(main_data['processed_daily_data']['w_force']['processed'])==False:
                    if pandas.isnull(main_fuel_cp_olmax)==False and pandas.isnull(w_force_olmin)==False:
                        if main_data['processed_daily_data']['main_fuel']['processed']>main_fuel_cp_olmax and main_data['processed_daily_data']['w_force']['processed']<=w_force_olmin:
                            ...
                            main_dict_list[i]['cp_message']=self.ship_configs['data']['main_fuel_cp']['rule_based_message']
                            print("doneeeeee")
                        else:
                            main_dict_list[i]['cp_message']=None
    ...
    def final_maindb_config(self,main_dict_list):
        ...
        for i in main_dict_list:
            maindb.insert_one(i).inserted_id
            print("inserted {}".format(j))
            j=j+1
        print("final done")
